...
class InterfaceHandler:
    ...
    def set_bslintrc_path(self):
        try:
            bslintrc_path = os.path.join(self.manifest_path, ".bslintrc")
            if bslintrc_path is not None:
                self.bslintrc = bslint.config_loader.read_json(bslintrc_path)
            else:
                raise FileNotFoundError
